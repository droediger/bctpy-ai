# Translated from rich_club_wd.m

import numpy as np

def rich_club_wd(CIJ, *varargin):
    """
    Rich club coefficients curve (weighted directed graph)
    ...
    NodeDegree = np.sum(CIJ != 0, axis=0) + np.sum(CIJ != 0, axis=1) #define degree of each node (indegree + outdegree)
